...
class Product extends CI_Controller {
	...
	function _setlist($data,$ismultiple=true){
		...
		foreach($newdata as $key=>$item){
			...
			//排序
			//<td width=50 align="center"><input type="hidden" name="ids[]" value="'.$item['id'].'"><input type="text" name="listorder[]" class="input-order" size="3" value="'.$item['listorder'].'"></td>
			//checkbox
			//<td width=30 align="center"><input type=checkbox name="optid[]" value='.$item['id'].'></td>
			$newstr.='<tr id="tid_'.$item['id'].'" class="active">
			
			<td style="text-align:center;">'.$item['title'].'</td>
			<td width=80 style="text-align:center;" id="'.$item['id'].'">'.$item['hits'].'</td>
			<td width=150 style="text-align:center;"><input style="padding:0;float:left;font-size:12px;" value="刷新" name="reflesh" onclick=\'refdata("'.trim($item['title']).'","'.$item['id'].'");\' type="button" class="btn btn-link" /><a style="padding-top:1px;float:left;" href="'.site_aurl('down/gethis/'.$item['id'].$this->langurl).'">历史</a></td></tr>';
		}
		return $newstr;
	}
	function getrank(){
		...
		$urls="http://www.baidu.com/s?wd=$enKeyword&&rn=50";//baidu
		...
		//print_r($contents);
		curl_close($ch);
		preg_match_all('/<div[^>]*?class="result c-container "[^>]*>[\s\S]*?<\/span>/i',$contents,$rs);
		//print_r($rs);
		foreach($rs[0] as $k=>$v){
			$px++;
			//print_r($v);
			//print_r($url);
			$url2=substr($url,0,12);
			if(strstr($v,$url2)){
				$rsState = true;
				preg_match_all('/<span[^>]*?class="g"[^>]*>[\s\S]*?<\/span>/i',$v,$rs_t);
				break;
			}
		}
		unset($contents);
		if($rsState === true)
			...
			$result = '50外';
		$editdata['hits'] = $result;
		$this->Data_model->editData(array('id'=>$id),$editdata);
		$addDown['title'] = $result;
		$addDown['category'] = $id;
		$addDown['createtime'] = time();
		$this->Data_model->addData($addDown,'down');
		echo $result;
			
			
		//echo _get_rank_by_keyword($keyword,$website);
	}
	function _get_rank_by_keyword($keyword,$url)
	{
		$px = 0;//echo $px.'-';
		$rsState = false;
		$enKeyword = urlencode($keyword);
