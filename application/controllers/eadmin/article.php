...
class Article extends CI_Controller {
	...
	public function add(){
		...
		if($post['action']==site_aurl($this->tablefunc)){
			...
			//$data['content'] = str_replace("\r",'',nl2br($data['content']));
			$time = time();
			...
			for($i=0;$i<count($arr);$i++){
				$pro['title'] = $arr[$i];
				$pro['category'] = $id;
				$pro['keywords'] = $data['keywords'];
				$pro_id=$this->Data_model->addData($pro,'product');
			}
	...
	function _setlist($data,$ismultiple=true){
		...
		foreach($newdata as $key=>$item){
			...
			$newstr.='<tr class="active" id="tid_'.$item['id'].'">
			<td align="center" width=30><input type=checkbox name="optid[]" value='.$item['id'].'></td>
			<td align="center" style="padding-top:4px;" width=90><input type="hidden" name="ids[]" value="'.$item['id'].'"><input style="padding-top:1px;padding-bottom:1px;height:25px;" type="text" name="listorder[]" class="input-order" size="3" value="'.$item['listorder'].'"></td>
			<td align="center" width=80>'.$categorystr.'</td>
			<td align="center" width=180><a href="'.site_aurl('product/getpro/'.$item['id'].$this->langurl).'/'.$item['keywords'].'" style="color:'.$item['color'].'">'.$item['title'].'</a></td>
			<td align="center" width=180>'.$item['keywords'].'</td>
			<td align="center">'.$item['func'].'</td></tr>';
		}
		return $newstr;
	}
}